...
st.sidebar.header("2. Color Range Tuning")
# Blue Hue range (Standard is ~90-130)
blue_hue = st.sidebar.slider("Blue Hue Range", 0, 180, (80, 135))
# Purple Hue range (Standard is ~130-170)
purple_hue = st.sidebar.slider("Purple Hue Range", 0, 180, (136, 175))

st.sidebar.header("3. Detection Sensitivity")
...
if uploaded_file is not None:
    ...
    # Using the slider values here
    lower_b = np.array([blue_hue[0], 40, 20])
    upper_b = np.array([blue_hue[1], 255, 255])
    mask_b = cv2.inRange(hsv, lower_b, upper_b)

    lower_p = np.array([purple_hue[0], 40, 20])
    upper_p = np.array([purple_hue[1], 255, 255])
